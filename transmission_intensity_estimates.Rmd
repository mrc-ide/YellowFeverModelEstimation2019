...
mcmc_out = get_chains("multi_model_MCMC_chain_20190903", 
                       burnin = 8e5, thin = 100)

# glm_mcmc_out <- YFestimation::get_chains("GLM_MCMC_chain_20190913_6_new_pop_nb",burnin =1.5e6, thin = 1)
glm_mcmc_out <- YFestimation::get_chains("GLM_MCMC_chain_20190915_6_new_pop_dat_wes",
                         burnin =2.5e6, thin = 100)
...
#read in models and get them in the right order of covariates
covar <- covar[grep("cases_or_outbreaks|family", covar, invert = TRUE)]

model_form <- paste0("cases_or_outbreaks~", paste(covar, collapse = "+") ,
...
foi = plot_transmission_intensity2(x,
                                   ii,
                                   seroout,
                                   params = Foi_param[,2],
                                   dat ,
...
```{r map_foi_by_country}

tmp = foi %>% group_by(adm0) %>% summarise(median_foi = median(FOI, na.rm = TRUE))
...
```{r FOI_by_risk, fig.align='center', fig.pos = 'h', strip.white=F, fig.height = 10}
...
for(i in 1:n_samples){
  ...
  params = bind_cols(vac_eff = exp(mcmc_out[sample_ind1, "vac_eff"]),
...
write.csv(foi_out, "FOI_samples.csv", row.names = FALSE)
```
...
if(exists("foi_out")){
  coeff_of_var = sapply(1:ncol(foi_out), FUN = function(i) sd(foi_out[,i])/mean(foi_out[,i])) *100
  names(coeff_of_var) = colnames(foi_out)
  
  colours = snapalette("Space", 100, type = "continuous")
  
  shp1$cv = NA
  shp1$cv = coeff_of_var[match(shp1$GID_1, names(coeff_of_var))]
  
  mybreaks= seq(min(0, na.rm = TRUE), max(shp1$cv, na.rm = TRUE)+0.01, length.out=101)
