...
glm_mcmc_out <- YFestimation::get_chains("GLM_MCMC_chain_20190930_step_1_wideprior",
                          burnin =1e5, thin = 1)
...
covar_family <- unlist(strsplit(unlist(strsplit(model_form_whole, "\\+")), "\\~"))

dat <- dat[, -which(!names(dat) %in% covar_family & grepl("family", names(dat)))] 

# get model covariates
covar = names(glm_mcmc_out %>% dplyr::select(-c(Intercept, starts_with("adm05"), posteriorProb, acceptRate))) 
...
write.csv(foi_out, "FOI_samples_step_1_wideprior.csv", row.names = FALSE)
