...
glm_mcmc_out <- YFestimation::get_chains("GLM_MCMC_chain_20190927_d",
                                         burnin =5e4, thin = 1)
...
model_form_whole <- read.csv("Model_form_d.csv", stringsAsFactors = FALSE)$x
...
covar = names(glm_mcmc_out %>% select(-c(Intercept, starts_with("adm05"), posteriorProb, acceptRate))) 
...
write.csv(foi_out, "FOI_samples_d.csv", row.names = FALSE)
