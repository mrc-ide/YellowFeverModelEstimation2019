...
shp0 = rgdal::readOGR("../../shapefiles/Africa_America_0_smooth.shp",
                          ...
                          encoding = "UTF-8",
                          use_iconv = TRUE)
shp1 = rgdal::readOGR("../../shapefiles/Africa_America_1.shp",
                          stringsAsFactors = FALSE,
                          encoding = "UTF-8",
                          use_iconv = TRUE)
...
dat %<>% filter( is.finite(as.numeric(MIR.max) ))
...
# add ranges
dat = dplyr::mutate(dat,
                    EVI.range = as.numeric(EVI.max) - as.numeric(EVI.min),
...
for(c in covar){
  parm = shp1@data[, c]
  mybreaks = seq(min(as.numeric(parm), na.rm = TRUE) - 1e-5, 
                 max(as.numeric(parm), na.rm = TRUE) + 1e-5, length.out=1001)
