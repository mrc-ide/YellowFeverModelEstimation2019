...
```{r posteriorProb, fig.cap = "Posterior porbability from entire MCMC chain, thinned by a factor 100."}
...
```{r loglike, fig.cap = "Posterior porbability from entire MCMC chain, thinned by a factor 100."}
GLMprior_rename = function(param) {
  
  Prior = rep(0,4)
  
  #GLM
  jj = grep("^adm05", names(param)) # select country parameters (parameter_type=2) the sd.prior=2 is from Kevin's original code create status
  ...
  Prior[4] = sum(log(dtrunc(param[grep("low_risk", names(param))],
                            ...
                            mean = 0,
                            sd = 30,
                            a = 0, b = Inf)), na.rm = T)# brute force the adm05 low risk to be positive
...
glm_mcmc_out$prior <-  sapply(1:nrow(glm_mcmc_out),
                            FUN = function(i){
