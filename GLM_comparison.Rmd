...
glm_mcmc_out = NULL
for(i in 1:20){
  tmp <- YFestimation::get_chains(paste0("GLM_MCMC_chain_20200122_bestglm_5_", 
                                         i),
                                  burnin =1e5, thin = 100)
...
for(i in 1:20){
  ...
  model_form_whole = read.csv("bestglm_5.csv", stringsAsFactors = FALSE) %>% 
...
for(i in 1:nrow(df_sample)){
  ...
  #remove families of NHP that are not to be included
  model_form_whole = read.csv("bestglm_4.csv", stringsAsFactors = FALSE) %>% 
    dplyr::select(-Criterion)
