...
glm_mcmc_out = NULL
for(i in 1:20){
  tmp <- YFestimation::get_chains(paste0("GLM_MCMC_chain_20200903_bestglm_A_", 
                                         i),
                                  burnin =1e5, thin = 1)
...
ggplot(glm_mcmc_out[sample_ind, ]) + 
  geom_violin(aes(x = variant, y = like, fill = variant)) +
  theme_bw()+
  theme(legend.position = "none")+
  xlab("Variant") +
  ylab("Log Likelihood") +
...
ggplot( aes(x = variant, y = AUC, fill = variant)) +
  geom_violin() +
  theme_bw()+
  theme(legend.position = "none") +
  xlab("Variant")+
...
write.csv(df_sample, "new_Sampled_glm_parameters.csv", row.names = FALSE)
write.csv(df_glm, "new_Sampled_glm_predictions.csv", row.names = FALSE)
