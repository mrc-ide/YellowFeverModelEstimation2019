...
foi = read.csv( "FOI_samples_diffadm05.csv", stringsAsFactors = FALSE)

ve = median(exp(mcmc_out$vac_eff))
...
infections_df %<>% mutate(prob_severe = rbeta(nrow(infections_df),
                                              6.367309,44.60736),
...
```{r boxplot_simple, fig.width=10, fig.height=8}
...
ggplot(df) + 
  geom_boxplot( aes(x = adm0, y = severe_infections))  + 
  theme(axis.text.x = element_text(angle = 90))
...
```{r severe_table}
reporting_propn = 120000/805 # 149  #Nasidi et al. 1989,
...
```{r reported_map, fig.cap="Map of countries where we may expect to see at least 1 case reported in 2018."}
