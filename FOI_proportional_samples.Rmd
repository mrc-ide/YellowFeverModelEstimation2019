...
knitr::opts_chunk$set(
  dev = "png",
  fig.path = paste0("new_images_bestglmA/"),
  dpi = 300,
  warning = FALSE,
  message = FALSE
)
...
if(!exists("mcmc_out")){
  ...
  write.csv(sample(exp(mcmc_out$vac_eff), 200), "vac_eff_samples.csv", row.names = FALSE)
}

glm_mcmc_out <- YFestimation::get_chains(paste0("GLM_MCMC_chain_20200903_bestglm_A_", 1),
                          burnin =1e5, thin = 1)
...
df_sample <- read.csv("new_Sampled_glm_parameters.csv", stringsAsFactors = FALSE)
...
write.csv(foi_out, "new_sampled_foi_predictions.csv", row.names = FALSE)
