...
burden_df %<>% left_join(burden_df_nomassvac)
...
write.csv(tmp2, paste0("new_Table_of_massvac_deaths_", params$year,  ".csv"), 
          row.names = FALSE)
```
...
df %>% group_by(countries) %>% 
  ...
  arrange(-diff_median) %>%
  write.csv( paste0("new_Table_of_massvac_impact_", params$year,  ".csv"), 
          row.names = FALSE)
...
df %>% group_by(countries) %>% 
  ...
  arrange(-diff_median) %>%
  write.csv( paste0("new_Table_of_massvac_impact_", params$year,  "_percent.csv"), 
          row.names = FALSE)
...
df %>% ungroup() %>%
  ...
  arrange(-diff_median) %>%
  write.csv( paste0("new_Table_of_massvac_impact_", params$year,  "_percent_all.csv"), 
          row.names = FALSE)
