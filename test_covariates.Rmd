...
data.frame(covariate = covar_to_test, 
                        pvalue = pvalues, 
                        coeff = coeffs,
                        BIC = BICs) %>% 
               arrange(-pvalues)
...
We examine covariates that are highly correlations.
...
We find that some covariates are highly correlated. As such, we group these into clusters where the abs piecewise correlation is above 0.75.
...
In order to reduce the number of covariates to test, we choose those covariates in each cluster that are most correlated with our outcome variable.
...
write.csv(out$BestModels, "bestglm_1.csv", row.names = FALSE)
```
