...
covar_to_test <- covar_to_test[grep("family", covar_to_test, invert = TRUE)]

covar_to_test <- covar_to_test[grep("EVI.max", covar_to_test, invert = TRUE)] # because EVI.range is so correlated with it
covar_to_test <- covar_to_test[grep("temp_max", covar_to_test, invert = TRUE)] # because EVI.range is so correlated with it
...
covar_to_test <- as.character( df$param[abs(df$correlation)>0.2])
...
#discard those that are chosen once or less
df_freq %<>% filter(Freq>1)
