...
knitr::opts_chunk$set(
  dev = "png",
  fig.path = paste0("new_images_bestglmA/", params$year, "_"),
  dpi = 300,
  warning = FALSE,
  message = FALSE
)
...
```{r read in foi}
foi_out <- read.csv("new_sampled_foi_predictions.csv", stringsAsFactors = FALSE)
...
write.csv(tmp2, paste0("new_Table_of_deaths_proportional_", params$year,  ".csv"), 
          row.names = FALSE)


tmp3 <- left_join(df, tmp)

tmp3 <- tmp3 %>% group_by(continent,sample) %>% 
  summarise(deaths = sum(deaths, na.rm = TRUE),
            severe_infections = sum(severe_infections, na.rm = TRUE))

write.csv(tmp3,  paste0("new_Continent_Table_of_deaths_proportional_", params$year,  ".csv"), 
          row.names = FALSE)
```
